...
function wxLogin(goto){
	plus.oauth.getServices(function(services){
		for(var k in services){
			if(services[k].id == 'weixin'){wxLoginObj = services[k];}
		}
		if(wxLoginObj == null){mui.toast('获取微信登录服务失败'); return ;}
		wxLoginObj.login(function(res){
			wxLoginObj.getUserInfo(function(){
				var openid   = wxLoginObj.authResult.unionid;
				...
				mui.post(
					...
					function(data){
						...
						plus.storage.setItem('suid', data.id+'');
						plus.storage.setItem('snickname',data.nickname);
						plus.storage.setItem('sface',face);
						plus.storage.setItem('srand',data.randnumber);
						mui.toast('登录成功');
...
function logoff(){
	plus.oauth.getServices(function(services){
		...
		wxLoginObj.logout(function(){
			...
			plus.storage.removeItem('snickname');
			plus.storage.removeItem('sface');
			plus.storage.removeItem('srand');
			mui.toast('退出成功');
			var _index = plus.webview.getLaunchWebview();
			_index.evalJS("changSub(1); h('#tabBar').find('a').removeClass('mui-active'); h('#nav1').addClass('mui-active');");
		}, function(e){
